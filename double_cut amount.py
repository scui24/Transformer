...
def  transformed_double_amount(ingredients, steps):
     transformed_ingredients = []
     for ingredient in ingredients:
         ingredient = re.sub(r"(\d+)(?=[a-zA-Z ])", lambda x: str(int(x.group())*2), ingredients)
     transformed_ingredients.append(ingredient)
    
     transformed_steps = []
     for step in steps:
         step = re.sub(r"(\d+)(?=[a-zA-Z ])", lambda x: str(int(x.group())*2), steps)
     transformed_steps.append(step)
